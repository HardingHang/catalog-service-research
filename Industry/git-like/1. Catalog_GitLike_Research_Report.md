...
**[参考 · 图1：Lakehouse 整体架构关系](cjy_src/Catalog_GitLike_Research_Report_src/fig1_architecture.html)**
...
**[参考 · 图2：不可变对象、可变指针与指针翻转](cjy_src/Catalog_GitLike_Research_Report_src/fig2_immutable_pointer.html)**（步骤 ① ② ③）
...
![图 2-1：Iceberg 单表 branch 示意](cjy_src/Catalog_GitLike_Research_Report_src/fig_2_1_iceberg_branch.png)
...
**[参考 · 图2：不可变对象、可变指针与指针翻转](cjy_src/Catalog_GitLike_Research_Report_src/fig2_immutable_pointer.html)**（步骤 ④ ⑤）  
**[参考 · 图3：Nessie Git-like 操作清单](cjy_src/Catalog_GitLike_Research_Report_src/fig3_operations.html)**  
**[参考 · 图4：并发 Commit 冲突时间线](cjy_src/Catalog_GitLike_Research_Report_src/fig4_concurrent_commit.html)**  
**[参考 · 图5：Merge Replay 与 WAP 流程](cjy_src/Catalog_GitLike_Research_Report_src/fig5_merge_wap.html)**
...
![图 3-2：Nessie 两张逻辑表结构](cjy_src/Catalog_GitLike_Research_Report_src/fig_3_2_nessie_tables.png)
...
![图 3-5：Merge Replay 示意](cjy_src/Catalog_GitLike_Research_Report_src/fig_3_5_merge_replay.png)
...
**[参考 · 图3：Nessie Git-like 操作清单](cjy_src/Catalog_GitLike_Research_Report_src/fig3_operations.html)**（点击各操作卡片查看实现伪代码与关键性质）
...
**[参考 · 图4：并发 Commit 冲突时间线](cjy_src/Catalog_GitLike_Research_Report_src/fig4_concurrent_commit.html)**（分步动画演示）
...
![图 4-2：原子边界与多租户隔离的取舍](cjy_src/Catalog_GitLike_Research_Report_src/fig_4_2_atomic_tenant.png)
...
本报告的配图分为两类，均存放在 `cjy_src/Catalog_GitLike_Research_Report_src/` 目录：

**静态 PNG 图**（文档内嵌，直接显示）：

| 文件 | 对应章节 |
|---|---|
| `cjy_src/Catalog_GitLike_Research_Report_src/fig_2_1_iceberg_branch.png` | §2.1 Iceberg 单表 branch 示意 |
| `cjy_src/Catalog_GitLike_Research_Report_src/fig_3_2_nessie_tables.png` | §3.2 Nessie 两张逻辑表结构 |
| `cjy_src/Catalog_GitLike_Research_Report_src/fig_3_5_merge_replay.png` | §3.5 Merge Replay 示意 |
| `cjy_src/Catalog_GitLike_Research_Report_src/fig_4_2_atomic_tenant.png` | §4.2 原子边界与多租户取舍 |

**交互式 HTML 图**（点击链接在浏览器中打开）：

| 文件 | 说明 |
|---|---|
| `cjy_src/Catalog_GitLike_Research_Report_src/fig1_architecture.html` | Lakehouse 整体架构（含角色说明）|
| `cjy_src/Catalog_GitLike_Research_Report_src/fig2_immutable_pointer.html` | 不可变对象 / 可变指针 / 指针翻转（5 步） |
| `cjy_src/Catalog_GitLike_Research_Report_src/fig3_operations.html` | Nessie Git-like 操作清单（可点击） |
| `cjy_src/Catalog_GitLike_Research_Report_src/fig4_concurrent_commit.html` | 并发 commit 冲突时间线（分步动画）|
| `cjy_src/Catalog_GitLike_Research_Report_src/fig5_merge_wap.html` | Merge Replay + WAP 完整流程 |
