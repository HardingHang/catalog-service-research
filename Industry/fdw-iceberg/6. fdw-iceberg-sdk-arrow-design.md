...
Java 侧的 Arrow buffer 不走 JVM 堆外分配，而是通过自定义 `BufferAllocator` 经 JNI 回调 C 侧的 `palloc`/`MemoryContext` 完成分配。批次结束时 C 侧直接 `MemoryContextReset()` 批量回收，无需等待 GC。

```
C 侧                                            Java 侧 (JNI 回调)
═══                                             ════════════════

// 1. 初始化：为 Arrow batch 创建专用 MemoryContext
arrow_cxt = AllocSetContextCreate(
    parent_cxt, "arrow_batch",
    ALLOCSET_DEFAULT_SIZES);

// 2. 注册 C 侧回调，传给 Java
jni_register_allocator(env,
    arrow_alloc,    // → 函数指针
    arrow_free,     // → 函数指针
    arrow_cxt);     // → MemoryContext 句柄

arrow_alloc(Size size):          // 被 Java 通过 JNI 回调
    old_cxt = MemoryContextSwitchTo(arrow_cxt)
    ptr = palloc(size)           // ← 分配在 arrow_cxt 上
...
// 3. 读一批数据
nrows = readNextBatch()  ──JNI──►  iterator.next()
                                    → ColumnarBatch
                                    → Data.exportVector(buf_allocator, fv, ...)
                                       │
                                       └► buf_allocator.allocate(size)
                                            │
                                            └─JNI回调─► arrow_alloc(size)
                                                          → palloc(size)

// 4. 整批回收
MemoryContextReset(arrow_cxt)   // 一步释放所有 Arrow buffer
releaseBatch()  ──JNI──►  c_array.close()  // 清理 Java 侧引用
```
...
- `iceberg_scan_next`：调 `jni_read_batch`，返回的 `ArrowArray` / `ArrowSchema` 其底层 buffer 由 `arrow_cxt` 管理（见 3.1.3），调用方读完一批后直接 `MemoryContextReset(arrow_cxt)` 回收
- `iceberg_scan_close`：调 `jni_close` + `MemoryContextDelete(arrow_cxt)` + `pfree`

#### 3.2.4 内存分配回调

C 侧提供 `alloc` / `free` 回调，在 JVM 初始化时通过 `RegisterNatives` 注册给 Java 侧（见 3.1.3）。底层走 `palloc` / `pfree`，由 PG `MemoryContext` 统一管理。
...
tuple IterateForeignScan(ForeignScanState *node):
    ...
    if fsstate->next_tuple >= fsstate->num_tuples:
        ...
        MemoryContextSwitchTo(old_cxt)
        // 回收由 Java → JNI → palloc 分配的 Arrow buffer（见 3.1.3/3.2.4）
        MemoryContextReset(arrow_cxt)
...
定点类型（int/long/float/double/date/time/timestamp/uuid）直接读 buffer 值赋 Datum，不做拷贝。变长类型（string/binary）必须 `palloc` 拷贝到 `batch_cxt`，因为 Arrow buffer 在 `MemoryContextReset` 后被回收。`decimal` 需额外转换：Iceberg 用 16 字节紧凑表示，GaussVector Numeric 用变长结构，需逐值展开。
...
**本方案**：理想方案为 C 侧 `palloc`/`MemoryContext` 作为 Arrow 分配后端。Java `BufferAllocator` 通过 JNI 回调走 `palloc`，`MemoryContextReset` 批量回收。FDW 无需拷贝变长数据。

**异同**：本方案比起 GV Formation 消除了 Java → C 的数据拷贝（变长数据不拷贝，定点数据直读）；接近直接 Parquet 路径的效率但保留了 Java SDK 的 Iceberg 语义。拷贝只在 `Decimal` 等需要格式转换的类型上发生。
