...
| 对象关系建模 | 统一对象图最容易起步，但复杂关系容易依赖 `type` 分支解释 | 统一对象层还在，同时可在类型扩展表中承载对象差异，最适合当前多对象关系场景 | 对象内关系最清楚，但跨类型关系通常要额外抽统一引用层 |
| API / 服务层复杂度 | 通用接口最容易抽象，但复杂对象语义容易堆进分支逻辑 | 统一资源接口和对象专属逻辑比较平衡 | 对象专属接口最清楚，但服务数量和接入工作通常更多 |
| 外部系统接入 | 快速接入最方便，但字段语义容易松散 | 既能统一接入，又能按类型承载差异 | 每类系统映射更清晰，但适配成本更高 |
| 事件 / 审计模型 | 统一事件最容易起步 | 统一事件 + 类型 payload 更平衡 | 对象事件清晰，但跨类型事件要额外统一 |
| 迁移与回滚成本 | 前期低，后期从弱结构迁出成本高 | 中等，可按类型逐步迁移 | 单对象迁移清晰，但对象类型多时迁移次数多 |
...
| 方案 | 演进优势 | 演进劣势 |
| --- | --- | --- |
| `统一主表 + 通用扩展表` | 前期扩对象和加低频字段最快，统一目录、统一权限挂点最容易起步 | 后期最容易从弱结构回拆，强语义、版本化、复杂查询一多就容易积累结构债务 |
| `统一主表 + 类型专属扩展表` | 演进最均衡，既能继续加对象类型，也能保留对象边界，并自然扩展子表和版本表 | 需要持续治理统一层和类型层边界，否则类型扩展表容易膨胀 |
| `独立主表` | 核心对象继续做深最自然，版本化、强约束、复杂查询和性能优化路径最清楚 | 横向扩对象类型最慢，后续如果要统一权限、治理、搜索，通常还要补统一层 |
